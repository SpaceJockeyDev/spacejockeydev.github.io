...
const TOUR_URL = "https://teenagebottlerocket.com/tour-dates/";
const ARTIST_NAME = "Teenage Bottlerocket";
...
const parseMonthDay = (raw) => {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    const monthKey = tokens[i].slice(0, 3).toLowerCase();
    if (MONTHS[monthKey]) {
      month = MONTHS[monthKey];
      const maybeDay = Number.parseInt(tokens[i + 1] ?? "", 10);
...
const inferYears = (events) => {
  ...
  return events.map((event) => {
    if (
      lastMonth !== null &&
      (event.month < lastMonth || (event.month === lastMonth && event.day < lastDay))
    ) {
      year += 1;
    }
...
const buildShow = (index, event) => {
  const startDate = `${event.year}-${pad(event.month)}-${pad(event.day)} ${DEFAULT_TIME}`;
  const formattedDate = formatDate(event.year, event.month, event.day);
  const venueLocation = parseLocation(event.venueLocation);

  const eventDate = new Date(`${event.year}-${pad(event.month)}-${pad(event.day)}T00:00:00`);
  ...
  try {
    ...
    const events = await page.$$eval(".bit-event", (rows) => {
      ...
      const mappedRows = rows.map((row) => {
        ...
        const ticketLink = row.querySelector("a[href*='bandsintown.com/t/']");

        return {
          dateText: getText(row, ".bit-date"),
          venueName: getText(row, ".bit-venue"),
          venueLocation: getText(row, ".bit-location"),
          details: getText(row, ".bit-details"),
...
const run = async () => {
  const rawEvents = await extractEvents();
  if (!rawEvents.length) {
  ...
  const parsed = rawEvents
    .map((event) => {
      const monthDay = parseMonthDay(event.dateText);
      if (!monthDay) {
        return null;
      }
      return {
  ...
  const eventsWithYear = inferYears(parsed);
  const newShows = eventsWithYear.map((event, index) => buildShow(index + 1, event));
